  ...
  int buf_len, long int timeout )
{
  ...
  do {
    ...
    mac_addr_str(eth_frame_ptr->dest_addr, macstr);
    //printf("\tdst mac = %s\n", macstr);
    mac_addr_str(eth_frame_ptr->src_addr, macstr);
    //printf("\tsrc mac = %s\n", macstr);
    mac_addr_str(iface->mac_address, macstr);
    //printf("\tiface mac = %s\n", macstr);
    //printf("\ttarget type %.4x\n", ntohs(eth_frame_ptr->type));
    is_target_type = (ntohs(eth_frame_ptr->type) == type);
    //printf("\tis_my_mac=%d, is_target_type=%d\n", is_my_mac, is_target_type);
